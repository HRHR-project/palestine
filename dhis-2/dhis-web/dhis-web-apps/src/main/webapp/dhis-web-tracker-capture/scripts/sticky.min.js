  ...
  module.directive('sticky', ['$window', function ($window) {
      return {
        ...
        link: function linkFn($scope, $elem, $attrs) {
          ...
          function transformTable(elem, remove){
            if(angular.isUndefined(remove) || remove === false){
                  addColumnWidth(elem);
            }
            else {
                removeColumnWidth(elem);
            }
          }
          
          function addColumnWidth(elem){
          ...
          function removeColumnWidth(elem){
              angular.forEach(elem.querySelectorAll('tr:first-child th'), function (thElem, i) {
                  thElem.style.width = "";
              });
          }
          ...
          $scope.stickElement = function () {
            ...
            if(angular.isDefined($scope.forTable) && $scope.forTable === true){
                transformTable($elem[0]);
            }            
            
            $elem
              .css('z-index', '10')
              .css('width', elem.offsetWidth + 'px')
              .css('height', elem.offsetHeight + 'px')
              .css('position', 'fixed')
          ...
          $scope.checkIfShouldStick = function () {
            ...
            //for table, check if table-height has changed            
            if(angular.isDefined($scope.forTable) && $scope.forTable === true){
                
                var parent = $elem.parent()[0];
                var parentHeight = parseInt(parent.offsetHeight);                                 
                stickyBottomLine = stickyLine + parentHeight - elem.offsetHeight;
            }
